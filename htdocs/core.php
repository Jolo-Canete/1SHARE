...
if (isset($_GET['requestId'])) {
    ...
    $query = "SELECT r.*, b.*, i.*, u.username, i1.itemID AS item1_id, i1.itemName AS item1_name, i1.itemImage_path AS item1_image, 
                     ...
                     i3.itemID AS item3_id, i3.itemName AS item3_name, i3.itemImage_path AS item3_image, r.requestID AS id,
                     i.userID AS ownerID
              FROM Request r
...
?>
        ...
        <?php if ($user_id == $ownerID) {
            echo '<div class="d-flex justify-content-center mt-4">';
            echo '<button type="button" class="btn btn-secondary ms-2" onclick="openRateItemPager(' . $requestID . ')">Rate Requester</button>';
            // Check if item 1 exists
            if ($item1_id) {
                echo '<button type="button" class="btn btn-primary ms-2" onclick="openRateItemPages(' . $item1_id . ')">Rate Item</button>';
            } else {
                // Disable button for item 1
                echo '<button type="button" class="btn btn-warning ms-2" data-bs-toggle="modal" data-bs-target="#rateModal" data-request-id="' . $requestid . '" disabled>Rate Item 3</button>';
            }

            // Check if item 2 exists
            if ($item2_id) {
                echo '<button type="button" class="btn btn-primary ms-2" onclick="openRateItemPages(' . $item2_id . ')">Rate Item</button>';
            } else {
                // Disable button for item 2
                echo '<button type="button" class="btn btn-danger ms-2" data-bs-toggle="modal" data-bs-target="#rateModal" data-request-id="' . $requestid . '" disabled>Rate Item 2</button>';
            }

            // Check if item 3 exists
            if ($item3_id) {
                echo '<button type="button" class="btn btn-primary ms-2" onclick="openRateItemPages(' . $item3_id . ')">Rate Item</button>';
            } else {
                // Disable button for item 3
                echo '<button type="button" class="btn btn-warning ms-2" data-bs-toggle="modal" data-bs-target="#rateModal" data-request-id="' . $requestid . '" disabled>Rate Item 3</button>';
            }

            echo '</div>';
        } else
            echo '<button type="button" class="btn btn-primary ms-2" onclick="openRateItemPage(' . $requestID . ')">Rate Item</button>';
    }
} else {
}
        ?>
        ...
        ?>
        <script>
            function openRateItemPage(itemValue) {
                // Construct the URL with item value
                var url = 'rateitem.php?requestID=' + encodeURIComponent(itemValue);
                // Open the URL in a new tab
                window.location.href = url;
            }
        </script>
         <script>
            function openRateItemPager(itemValue) {
                // Construct the URL with item value
                var url = 'rateresident.php?requestID=' + encodeURIComponent(itemValue);
                // Open the URL in a new tab
                window.location.href = url;
            }
        </script>
         <script>
            function openRateItemPages(itemValue) {
                // Construct the URL with item value
                var url = 'rateItemBarter.php?itemID=' + encodeURIComponent(itemValue);
                // Open the URL in a new tab
                window.location.href = url;
            }
        </script>
