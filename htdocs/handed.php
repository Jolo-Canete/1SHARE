    ...
    if ($uploadOk == 1) {
        if (move_uploaded_file($_FILES["proof"]["tmp_name"], $targetFile)) {
            ...
            if ($userId == $requestUserId) {
                $sql = "UPDATE borrow SET RequesterProof = '$uniqueFileName', handed = 'Yes' WHERE requestID = '$requestId'";
            } else {
                $sql = "UPDATE borrow SET OwnerProof = '$uniqueFileName', handed = 'Yes' WHERE requestID = '$requestId'";
            }
